...
//http库一共有四个类，HttpRequest（http请求类封装）、HttpResponse（http响应类封装）、HttpContext（http协议解析类）、HttpServer（http服务器封装）。


// 下面的函数分别属于上述当中的四种
const __uint32_t DEFAULT_EVENT = EPOLLIN | EPOLLET | EPOLLONESHOT;
const int DEFAULT_EXPIRED_TIME = 2000;    // ms 默认的过期时间是2000ms,
const int DEFAULT_KEEP_ALIVE_TIME = 5 * 60 * 1000; // ms  默认 ，默认的持久连接时间为5分钟， 300秒 然后就是 300000微妙
...
//httpData 封装一个EventLoop， 然后客户端和服务端的交互处理就是在这个EventLoop 当中完成, 构造函数

// 每个用户请求都会被封装成为httdata，初始化一些eventLoop 和channel, fd 等
//
...
void HttpData::handleConn()
{
    ...
    if (!error_ && connectionState_ == H_CONNECTED)
    {
        if (events_ != 0)
        {
            ...
            events_ |= EPOLLET;
            // 边缘触发模式， 每处理一个连接，都需要进行进行调用epoll_clt 进行事件注册
...
// http 解析协议 ,处理请求行 request line 其中URI 就是在请求行当中
